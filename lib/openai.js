...
function estimateTokenCount(messages) {
  return messages.reduce((total, msg) => {
    ...
    return total + Math.ceil(contentLength / 4); // 1 токен ≈ 4 символа
  }, 0);
}

// Получаем лимит токенов из OpenRouter API
async function fetchTokenLimit() {
  const now = Date.now();
  if (cachedTokenLimit && now - lastTokenFetchTime < 60000) {
    return cachedTokenLimit; // кэш: 60 секунд
  }
  ...
  cachedTokenLimit = Math.max(500, tokensAvailable); // минимум 500
